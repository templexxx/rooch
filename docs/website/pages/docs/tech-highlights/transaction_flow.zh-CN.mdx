...
4. Sequencer 承诺 L2 output commitment 提交的最大间隔为 t3。 t3 << t2，以确保 L2 上的 L1 区块信息更新 tx 的执行结果可用于仲裁，同时也意味着该结果可被 verifier 验证和挑战以确保区块信息的正确性。L1 合约会检查每次 output commitment 提交的间隔。
