...
1. L1 信息在 L2 上的审计跟踪：
    ...
    * 至少一个诚实的 verifier 对 Sequencer 提交的交易包含证明进行验证。
...
1. 提供 L1 最新区块头信息，及其状态证明
（此证据会在合约中保存一个挑战期，以供 verifier 通过比较 L2 提交到 L1 上的 State Commitment 验证结果，避免 Sequencer 提供伪证。此步骤不在本仲裁合约内，但为本合约的正确执行提供了保障，它属于 verifier 的验证工作的一部份。）。
2. 若最新区块头的区块时间不满足 t2，惩罚 Sequencer，挑战终止。用户后续可每隔 t1 时间持续性发起同一交易的包含仲裁，Sequencer 的不作为将面临逐步加重的惩罚[^1]。 
3. 提供该 L1ToL2 事件所在区块高度的区块头，及其状态证明 （此证据会在合约中保存一个挑战期，接受 verifier 挑战）。
...
7. 仲裁合约验证累加器证明及其是否满足 t1 承诺（累加器的根作为 L2 output 的一部份会携带提交时 L2 记录的最新 L1 的区块头信息一并提交至 L1），若验证失败，惩罚 Sequencer。
